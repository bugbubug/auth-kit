...
export function createOidcVerifier(
  ...
  async function verify(idToken: string): Promise<VerifyOidcResult> {
    ...
    try {
      ...
      throw new AuthKitError(
        "jwks_failure",
        "JwksSource.getKeys failed while verifying an OIDC id_token",
        { cause },
      );
    }
...
function fail(reason: OidcFailureReason): VerifyOidcResult {
 ...
 */
function mapJoseError(err: unknown): OidcFailureReason {
